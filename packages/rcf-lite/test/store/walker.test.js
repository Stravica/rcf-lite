...
test('walkTree on the live tree loads every document and returns zero errors', async () => {
  ...
  assert.equal(tree.requirements.length, 87); // H-1 REQ-145
  ...
  assert.equal(tree.userStories.length, 144); // H-1 US-14501
  ...
  assert.equal(tree.fbsItems.length, 64); // H-1 FBS-165
  ...
  // 2026-09-08 (118 -> 119).
  assert.equal(tree.testSuites.length, 120); // H-1 TS-175
...
test('walkTree computes childrenByParent by inversion (PRD has REQ-001..REQ-030 plus REQ-040, REQ-041, REQ-042, REQ-043, REQ-050..REQ-054, REQ-060..REQ-063, REQ-070..REQ-077, REQ-080..REQ-086, REQ-090..REQ-094, REQ-100..REQ-104, REQ-110..REQ-115, REQ-120..REQ-125, REQ-130..REQ-134, REQ-140, REQ-145)', async () => {
  const { tree } = await walkTree({ projectRoot: repoRoot });
