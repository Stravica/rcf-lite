...
test('walkTree on the live tree loads every document and returns zero errors', async () => {
  ...
  assert.equal(tree.requirements.length, 18);
  ...
  // (46 -> 47).
  assert.equal(tree.userStories.length, 47);
  ...
  // covering US-1801 (36 -> 37).
  assert.equal(tree.fbsItems.length, 37);
  ...
  // (46 -> 47).
  assert.equal(tree.testSuites.length, 47);
