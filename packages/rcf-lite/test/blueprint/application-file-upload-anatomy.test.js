// Anatomy + apply + probe-pack test for the application-file-upload v1.2.7 shelf blueprint (spec section 5.2).
//
// Covers TS-054.
...
test('application-file-upload: the contributions/probes/ pack carries probe-utils and every named probe with its run-*.mjs wrapper (TC-crit-e-pack-shape)', async () => {
...
test('application-file-upload: every probe module in the pack exports an anchorReqId naming a contributed REQ and accountBound=false (TC-crit-e-anchors)', async () => {
...
test('application-file-upload: every probe result in the pack carries one of the four 7d evidence shapes (TC-crit-e-evidence-shape)', async () => {
 ...
 _acIds.add('application-file-upload-' + ac.id);
 }
 }
 }
 ...
 runResultShapeNegativeCases({ familySlug: 'application-file-upload' });
});
