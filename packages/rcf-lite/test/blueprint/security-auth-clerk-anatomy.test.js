...
test('security-auth-clerk anatomy: helper refuses fabricated AC ids in limitation', () => {
  ...
  const wrongSlug = classifyShape({ conformanceOnly: true, anchorAcId: null, limitation: 'wrong-slug-AC-9101-1: real bare id under a fake slug' }, opts);
  assert.equal(wrongSlug.ok, false, 'wrong-slug prefix on a real bare AC must be refused');
  const suffixed = classifyShape({ conformanceOnly: true, anchorAcId: null, limitation: 'security-auth-clerk-AC-9101-1bogus: trailing garbage after the AC id' }, opts);
