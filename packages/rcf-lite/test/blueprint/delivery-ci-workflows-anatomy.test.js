// Anatomy test for the delivery-ci-workflows v2.3.2 shelf blueprint.
...
const BLUEPRINT_ROOT = join(REPO_ROOT, 'blueprints', 'delivery-ci-workflows');
const PROBES_DIR = join(BLUEPRINT_ROOT, 'contributions', 'probes');
const FIXTURE_DIR = join(REPO_ROOT, 'packages', 'rcf-lite', 'test', 'fixtures', 'probe-pack-delivery-ci-workflows');

const PROBES = ['workflow-template-shape', 'node-gate-entrypoint', 'real-account-github-actions-run-record'];

test('delivery-ci-workflows: blueprint.json version pinned at 2.3.2 (TC-crit-e-blueprint-json-version)', async () => {
  const doc = JSON.parse(await readFile(join(BLUEPRINT_ROOT, 'blueprint.json'), 'utf8'));
  assert.equal(doc.slug, 'delivery-ci-workflows');
  assert.equal(doc.version, '2.3.2');
});
...
test('delivery-ci-workflows: DECLARED_ENV covers every process.env read across probes + fixture src (TC-crit-e-env-derivation)', async () => {
...
test('delivery-ci-workflows: fixture README declares every env var in DECLARED_ENV (TC-crit-e-fixture-env-declaration)', async () => {
...
test('delivery-ci-workflows: every probe returns results whose rows each carry a 7d evidence shape or an honest skip (TC-crit-e-result-shape)', async () => {
  for (const p of PROBES) {
    const mod = await importProbe(join(PROBES_DIR, p + '.mjs'));
