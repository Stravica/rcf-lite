...
test('delivery-ci-workflows: blueprint.json version pinned at 2.3.2 (TC-e-mixed-blueprint-json-version)', async () => {
  const doc = JSON.parse(await readFile(join(BLUEPRINT_ROOT, 'blueprint.json'), 'utf8'));
  assert.equal(doc.slug, 'delivery-ci-workflows');
  assert.equal(doc.version, '2.3.2');
});

test('delivery-ci-workflows: contributions/probes/ pack is present and every probe declares anchor + accountBound (TC-e-mixed-probe-pack)', async () => {
...
test('delivery-ci-workflows: fixture README declares every env var the probes read (TC-e-mixed-fixture-env-declaration)', async () => {
