// Anatomy test for the persistence-data-d1 v1.1.4 shelf blueprint.
...
const BLUEPRINT_ROOT = join(REPO_ROOT, 'blueprints', 'persistence-data-d1');
const PROBES_DIR = join(BLUEPRINT_ROOT, 'contributions', 'probes');
const FIXTURE_DIR = join(REPO_ROOT, 'packages', 'rcf-lite', 'test', 'fixtures', 'probe-pack-persistence-data-d1');

const PROBES = ['facade-round-trip', 'migrations-forward-only', 'real-account-d1-round-trip'];

test('persistence-data-d1: blueprint.json version pinned at 1.1.4 (TC-crit-e-blueprint-json-version)', async () => {
