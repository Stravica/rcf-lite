...
test('observability-probe-endpoints: blueprint.json version pinned at 1.2.2 (TC-e-mixed-blueprint-json-version)', async () => {
  const doc = JSON.parse(await readFile(join(BLUEPRINT_ROOT, 'blueprint.json'), 'utf8'));
  assert.equal(doc.slug, 'observability-probe-endpoints');
  assert.equal(doc.version, '1.2.2');
});

test('observability-probe-endpoints: contributions/probes/ pack is present and every probe declares anchor + accountBound (TC-e-mixed-probe-pack)', async () => {
...
test('observability-probe-endpoints: fixture README declares every env var the probes read (TC-e-mixed-fixture-env-declaration)', async () => {
