// Anatomy + apply + probe-pack test for the application-forms-wizard v1.2.6 shelf blueprint (spec section 5.3).
//
// Covers TS-055.
...
const _CRIT_E_BP_ROOT = _joinCritE(_CRIT_E_REPO_ROOT, 'blueprints', 'application-forms-wizard');
const _CRIT_E_PROBES_DIR = _joinCritE(_CRIT_E_BP_ROOT, 'contributions', 'probes');
const _CRIT_E_FIXTURE_DIR = _joinCritE(_CRIT_E_REPO_ROOT, 'packages', 'rcf-lite', 'test', 'fixtures', 'probe-pack-application-forms-wizard');
const _CRIT_E_PROBE_NAMES = ["task-list-surface","step-page-shape","draft-persistence-round-trip"];

test('application-forms-wizard: the contributions/probes/ pack carries probe-utils and every named probe with its run-*.mjs wrapper (TC-crit-e-pack-shape)', async () => {
...
test('application-forms-wizard: every probe module in the pack exports an anchorReqId naming a contributed REQ and accountBound=false (TC-crit-e-anchors)', async () => {
...
test('application-forms-wizard: sample-app fixture README declares env vars for the probe pack (TC-crit-e-fixture-env-vars)', async () => {
...
test('application-forms-wizard: every probe result in the pack carries one of the four 7d evidence shapes (TC-crit-e-evidence-shape)', async () => {
 ...
 _acIds.add('application-forms-wizard-' + ac.id);
 }
 }
 }
 ...
 runResultShapeNegativeCases({ familySlug: 'application-forms-wizard' });
});
