...
test('email-smtp-resend: blueprint.json version pinned at 1.1.4 (TC-e-mixed-blueprint-json-version)', async () => {
  const doc = JSON.parse(await readFile(join(BLUEPRINT_ROOT, 'blueprint.json'), 'utf8'));
  assert.equal(doc.slug, 'email-smtp-resend');
  assert.equal(doc.version, '1.1.4');
});

test('email-smtp-resend: contributions/probes/ pack is present and every probe declares anchor + accountBound (TC-e-mixed-probe-pack)', async () => {
...
test('email-smtp-resend: fixture README declares every env var the probes read (TC-e-mixed-fixture-env-declaration)', async () => {
