// Anatomy test for the email-smtp-resend v1.1.5 shelf blueprint.
...
const BLUEPRINT_ROOT = join(REPO_ROOT, 'blueprints', 'email-smtp-resend');
const PROBES_DIR = join(BLUEPRINT_ROOT, 'contributions', 'probes');
const FIXTURE_DIR = join(REPO_ROOT, 'packages', 'rcf-lite', 'test', 'fixtures', 'probe-pack-email-smtp-resend');

const PROBES = ['smtp-round-trip', 'unverified-sender-refusal', 'real-account-resend-send'];

test('email-smtp-resend: blueprint.json version pinned at 1.1.5 (TC-crit-e-blueprint-json-version)', async () => {
...
test('email-smtp-resend: DECLARED_ENV covers every process.env read across probes + fixture src (TC-crit-e-env-derivation)', async () => {
...
test('email-smtp-resend: fixture README declares every env var in DECLARED_ENV (TC-crit-e-fixture-env-declaration)', async () => {
...
test('email-smtp-resend: every probe returns results whose rows each carry a 7d evidence shape or an honest skip (TC-crit-e-result-shape)', async () => {
  for (const p of PROBES) {
    const mod = await importProbe(join(PROBES_DIR, p + '.mjs'));
