...
const BLUEPRINT_ROOT = join(REPO_ROOT, 'blueprints', 'observability-essentials');
const PROBES_DIR = join(BLUEPRINT_ROOT, 'contributions', 'probes');
const FIXTURE_DIR = join(REPO_ROOT, 'packages', 'rcf-lite', 'test', 'fixtures', 'probe-pack-observability-essentials');

const PROBES = ['liveness-probe', 'readiness-probe', 'metrics-endpoint'];

test('observability-essentials: blueprint.json version pinned at 2.1.5 (TC-crit-e-blueprint-json-version)', async () => {
  const doc = JSON.parse(await readFile(join(BLUEPRINT_ROOT, 'blueprint.json'), 'utf8'));
  assert.equal(doc.slug, 'observability-essentials');
  assert.equal(doc.version, '2.1.5');
});
...
test('observability-essentials: DECLARED_ENV covers every process.env read across probes + fixture src (TC-crit-e-env-derivation)', async () => {
...
test('observability-essentials: fixture README declares every env var in DECLARED_ENV (TC-crit-e-fixture-env-declaration)', async () => {
...
test('observability-essentials: every probe returns results whose rows each carry a 7d evidence shape or an honest skip (TC-crit-e-result-shape)', async () => {
