...
| `CF_API_BASE` | override | Cloudflare REST API base URL the real-account probe posts to (default `https://api.cloudflare.com/client/v4`; overridden per CI runner or a mock endpoint). Declared code default; the fixture manifest holds the literal so the probe source is free of operational endpoint hosts. | `retry-and-fail-real-account.mjs` |
