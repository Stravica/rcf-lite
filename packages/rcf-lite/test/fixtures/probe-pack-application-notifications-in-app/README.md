...
| Var | Purpose |
|---|---|
| `PORT` | default 3000; probe picks 47640-47649 |
| `PROBE_BREAK` | optional default `?break=` switch across every request; per-request `?break=` still wins when both are set. Values: `preseed`, `role`, `timeout`, `ack` |
...
`blueprints/application-notifications-in-app/contributions/probes/` boots this fixture on a scratch port in its declared family range and drives varied inputs (different query strings and env overlays) to derive DOM observables. Each probe result carries an `evidence` object with the fixture's request id, the HTTP status and a response-body excerpt. No account credentials are involved: this blueprint's deliverable is application code and the fixture built from its own contributions IS the engine (the application-code engine rule).
...
- `POST /api/notifications/acknowledge`: the acknowledge round-trip endpoint the JS-on client script drives. Body `{ notificationId }`. Marks the row's `acknowledgedAt` on the delivery log and returns `{ ok: true, notificationId }`. Refuses with HTTP 502 `{ ok:false, error:"ACKNOWLEDGE_ROUND_TRIP_REFUSED" }` under `?break=ack` (or `PROBE_BREAK=ack`).
- `POST /actions/acknowledge-notification`: the progressive-enhancement acknowledge action the served per-item form submits with JS off (each centre item wraps its acknowledge button in `<form method="post" action="/actions/acknowledge-notification?notification-id=...">`; the JS-on client script intercepts the click, calls `preventDefault` on the form, and POSTs to `/api/notifications/acknowledge` instead so exactly one POST fires per activation). Reads `notification-id` from a form-encoded body first and falls back to the URL query. Marks the row's `acknowledgedAt` on the delivery log and returns `{ ok: true, notificationId, acknowledgedAt }`. Refuses with HTTP 502 `{ ok:false, error:"ACKNOWLEDGE_FORM_REFUSED" }` under `?break=ack` (or `PROBE_BREAK=ack`) exactly as the JSON API does; the query-scoped switch rides into the served form action so a JS-off submission originating from `/notifications-centre?break=ack` still reaches the refused branch.
...
| `?break=ack` | The JS-on acknowledge click handler is a no-op (no POST, no DOM update) AND both server-side acknowledge paths refuse with HTTP 502: the JSON API `POST /api/notifications/acknowledge` returns `{ ok:false, error:"ACKNOWLEDGE_ROUND_TRIP_REFUSED" }` and the progressive-enhancement form action `POST /actions/acknowledge-notification` returns `{ ok:false, error:"ACKNOWLEDGE_FORM_REFUSED" }` (the query-scoped switch rides into the served form action so a JS-off native form submission from `/notifications-centre?break=ack` reaches the refused branch too). | `AC-20103-1` centre acknowledge round-trip | block |
