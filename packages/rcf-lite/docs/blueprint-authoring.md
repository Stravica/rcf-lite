...
Packs receive `browser` and `fetch` through the runner's injected dependencies. The runner's pinned Playwright MCP (`packages/rcf-lite/src/verify/engine/launcher.js`) is the browser access seam; a pack MUST NOT close the browser it receives. `run` functions are expected to be pure with respect to the runtime state they leave behind; a check that mutates persistent state on the app under test without a cleanup path is refused at author-side review.

The dev server is assumed running; `boot` is an optional hint the operator opts into with `rcf verify browser --boot` (a future minor). A blueprint that ships a per-blueprint dev-server boot harness fails author-side review.
