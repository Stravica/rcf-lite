...
RCF's spec-side chain (PRD -> REQ -> US -> AC -> TS -> TC) is a closed graph: every id resolves to a document `rcf define validate` has loaded, so a broken link is mechanically detectable. Source code sat outside that graph. A test case's `testPointer` was a free-text string the walker could not follow (it has since gained its own working-tree check on the test axis: `rcf audit coverage` counts a TC only when its pointer resolves to a real test); nothing connected an acceptance criterion to the file that actually implements it. A path recorded as a plain string - the sidecar approach - gives you a pointer with no graph node behind it, so a rename or a deletion leaves a dangling reference that nothing checks until something downstream fails.

**Code Nodes make source code a first-class node in the same graph.** The same `rcf define validate` that guards the spec graph now also guarantees every code link resolves against the working tree. One graph, one walker, one query surface - extended, not duplicated.
...
`rcf define validate` checks every Code Node's `path` against the working tree:
...
```sh
rcf define validate --json
# {"kind":"staleCode","rule":"fileResolves","id":"CN-010", ...}
rcf define update CN-010 --set path=src/new-location.js#renamedFn
rcf define validate
# rcf define validate: tree is clean.
```

Large trees, or CI stages that want the split, can skip the staleness pass with `rcf define validate --no-code` (spec-graph checks only). The default remains full validation - a floor is only a floor if it runs by default.

## 5. Queries: trace, impact, and CN as a pivot

- **`rcf audit trace <path>`** and **`rcf audit trace <path>#symbol`** resolve a source path to its Code Node(s) and trace backward: path -> CN -> AC -> US -> REQ -> PRD. Multiple matches (a file-level CN and a symbol-level CN over the same file) print as separate blocks.
...
- **A Code Node id is a uniform pivot**, like any other: `rcf audit trace CN-006 --forward --to-code` walks the dependency blast radius; `rcf audit impact CN-006` works the same way any other id does.
- **`rcf audit view`** renders Code Nodes as a distinct cosmetic class in the per-requirement mermaid diagrams, hanging off the ACs they implement.
...
```sh
rcf define create cn --path src/store/validator.js#getAjv --acs AC-701-3
rcf define update CN-001 --set path=src/store/validator.js#getAjvBuilt   # repair path, same as any other kind
rcf define delete CN-001            # refused while another CN depends on it
rcf define delete CN-001 --cascade  # drops the dependency edge from dependents first
```
...
```sh
rcf define create cn --path src/a.js --derive-deps
...
`rcf audit coverage --with-code` classifies every acceptance criterion:
