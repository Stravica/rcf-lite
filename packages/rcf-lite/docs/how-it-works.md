...
Thirty subcommands under the single `rcf` bin, grouped by job:
...
The review surface itself has five tabs: overview, requirements, architecture, build sequence, and a Product Map that re-buckets the requirements without changing them - by shape (webUi, httpApi, auth, persistence, notifications, business rules), by component (rooted at the TAD, drilling TAC to US to REQ), by trace coverage (complete vs missing stories, AC scope, component owner or tests), by capability slug (see [capability-tags.md](capability-tags.md) for the convention), and by blueprint (one bucket per applied blueprint plus a Project bucket, drilling capability then REQ then US then AC). Every REQ row an applied blueprint contributed carries a compact blueprint badge in its summary in every grouping. A status filter on the Product Map applies to whichever grouping is active. Deep-linkable: a URL of the form `#tab=product-map&group=blueprint&status=approved` opens the tab in the same state a reviewer left it in.
