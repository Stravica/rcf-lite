...
- **`@stravica-ai/rcf-lite-core/patterns/services`.** The nine-category third-party service seed set (`SERVICE_SEED_PATTERNS_V1`: `emailDelivery | payment | smsVoice | auth | storageCdn | llmAi | analyticsTelemetry | featureFlags | search`), plus `SERVICE_CATEGORY_KEYS`, `GENERIC_SERVICE_VERBS` and a `matchServiceSignals(text)` helper that returns per-hit provenance (`{ category, role, pattern, match, index }`) in document order. Transcribed verbatim from Track A `verification-integrity-cluster-spec` §4.4; single source of truth for build's pre-flight service scanner (§8.3) and verify's provisioning heuristic (§5.3 / §8.4) — the two consumers that would otherwise drift, as they did in d-2026-07-30-142 when verify's `SERVICE_PATTERNS` regex missed `email channel`.
...
- **Package exports.** New subpaths: `./patterns/ui-shapes`, `./patterns/req-shapes`, `./patterns/services`, `./patterns/register-canary`, `./baseline-catalog`, `./fixtures/register-canary/*` (glob export for the fixture JSON files).
