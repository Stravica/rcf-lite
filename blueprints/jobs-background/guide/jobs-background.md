...
Applies 22 contributions and writes the sidecar `rcf/blueprints/jobs-background.applied.json` with `appliedCapabilities: ["queue"]` (discovered through the capability mechanism).
...
Boot line for local development against the in-memory queue-driver seam:
