...
From v2.0.0 of observability-essentials the two blueprints compose cleanly on the current shelf. Essentials v2 dropped its `scope: global` claims on the `healthProbes` and `readinessSemantics` topics and defers probe-path binding to this blueprint; probe-endpoints is now the sole `scope: global` claimant on both topics, so applying either order (essentials then probe-endpoints, or probe-endpoints then essentials) resolves to exactly one global ADR per topic with no `globalAdrTopic` conflict fired. The essentials blueprint continues to own `statusPageContract` and the notification-outcome sink; probe-endpoints continues to own the probe surface. The compose test at `packages/rcf-lite/test/blueprint/probe-path-alignment.test.js` is the ratified truth for this alignment.

Historical migration note (pre-v2 essentials): earlier essentials releases (v1.x, superseded by v2.0.0) claimed `scope: global` on `healthProbes` and `readinessSemantics`. A project that still applies pre-v2 essentials alongside this blueprint fires the two conflicts and resolves them at project level with a project-level ADR (adopt one shape, keep the existing, author a project-level ADR that supersedes both, or declare the resolution on the add itself with `--resolve`). Migrating to essentials v2 removes the resolution burden.
