...
- completion-persistence (AC-26104-1) upgraded from a de-claim to a positive-evidence row. The fixture now derives a server-side first-run flag on `/tour` when the applied `theme-persistence` store is `server-side-per-principal`: pages render `data-tour-first-run="true"` when the principal has no completion record and `data-tour-first-run="false"` when a completion record is present (`data-tour-completion-source` and `data-tour-principal-id` name the source). The probe drives the write, observes the flip on the next `/tour` request, drives the DELETE, and observes the flag return to `true` on a further `/tour` request; combined with the existing completion-store cycle (POST -> GET -> DELETE -> GET) and the settings-surface `data-action="restart-tour"` reachability, the probe now positively observes AC-26104-1's persistence + restart contract server-side. First-run-detection and stepper-role-dialog rows keep `notObservableHere` on the browser-only halves. Fixture README env-var manifest now declares `PROBE_BREAK` (the fixture reads it as an alternate to a per-request `?break=`) and the probe-utils `DECLARED_ENV` list mirrors the addition. Anatomy shape check tightened to reject an anchored `conformanceOnly` row; a new negative-case test constructs an anchored `conformanceOnly` row and asserts the shape helper throws.
