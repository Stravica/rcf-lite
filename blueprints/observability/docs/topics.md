...
Note on the delineation from the REST blueprint's `logging` topic: `logging` (owned by REST ADR-304) governs the wire-log shape of the HTTP tier. This blueprint's notification outcome sink (ADR-805) is a durable record surface for notification attempts, not a log; the two do not overlap.

Note on the delineation from the persistence blueprint's `persistenceStore` topic: this blueprint's notification outcome sink is substrate-agnostic. When composing with the persistence blueprint, the outcome sink's substrate is a facade verb on that store; the observability blueprint does not itself hold an opinion on the store engine and does not conflict on `persistenceStore`.

Rules for new topics (inherited from the SPA, REST, auth, and persistence vocabularies, restated as law): lower camel case, one concept per topic, no version suffixes. A topic names the decision area, not the chosen answer. Do not mint variants of existing strings (`healthEndpoints`, `probes`, `livenessReadiness` are all wrong when `healthProbes` already exists; `readinessAggregation`, `readyPolicy` are all wrong when `readinessSemantics` already exists; `statusPage`, `publicStatus`, `uptimePage` are all wrong when `statusPageContract` already exists).
...
| ci-pipeline | 6101-6899 | 7xx | in progress (HQ seat) | candidates (verify at that seat): `ciGates`, `strictCoverageGate` |
| observability (this package) | 7101-7899 | 8xx | queued final seat | `healthProbes`, `readinessSemantics`, `statusPageContract` (this package) |

US 7101-7110 sit at the LOW end of the 7101-7899 band on purpose. A project-side story that mechanically derives from an observability REQ id into the number `7110` would collide against observability-US-7110 in this package; the band leaves headroom at the HIGH end (US 7181-7899) so a project's own stories anchored to observability REQs can allocate without conflict. The watchpost run4 lesson applies here too.

Row-status caveat: the ci-pipeline seat is running in parallel at HQ on a disjoint branch; row statuses in this table reflect the queue state at authoring time. Whichever of the two PRs merges second updates the ci-pipeline and observability rows to their final `shipped v1.0.0` status; Dave coordinates the merge order.
