...
- Anatomy assertions are now semantic: every result row's evidence must carry an engine-minted identifier (`id`, `serverId`, `snapshotId`, `firewallId`, `imageId`, `containerId`, `requestId`, `vendorRequestId`, `resourceId`, or the deterministic `contentSha256` of a rendered / scanned artefact) OR a supplied/echo pair (`supplied<Name>` strictly equal to `echoed<Name>`), AND a derived observation (excerpt, statusCode, mode, wallClockTime, non-zero count, or structured engine-returned object). Event names, file paths, service names, manifest names, resource names, and probe inputs count only as derived context, never as the identifier.
- `caddyfile-validate`, `compose-config-lint` and `secrets-as-files-scan` carry `contentSha256` (deterministic hash of the observed artefact) on every row.
