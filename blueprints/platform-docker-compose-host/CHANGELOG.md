...
- Colour on E: AMBER on every AC whose only observation is offline (`AC-composeHost-secretsAreFiles`, `AC-composeHost-restartClassification`, `AC-composeHost-logDriverClassification`, the reverse-proxy-artefact and read-only-bind-mount clauses of `AC-38107-4` / `AC-composeHost-reverseProxyArtefactValid`); GREEN on the live-observed clauses (`AC-composeHost-upClean`, `AC-composeHost-zeroDowntimeReload`, `AC-composeHost-secretShape` in-container mode with a real 64-hex containerId, `AC-composeHost-healthcheckLint` for the `docker compose ps` healthy-service observation on the applied stack).

## 1.1.7 - 2026-09-11

- Anatomy assertions moved from key-name allow-lists towards a semantic rule: every result row's evidence must carry an identifier (a vendor / resource id, engine request id, or a supplied/echo pair with strict equality) AND a derived observation (excerpt, statusCode, mode, wallClockTime, non-zero count, or structured engine-returned object). Event names, file paths, service names, manifest names, resource names, and probe inputs are derived context, not identity. Note: this pass still accepted a probe-computed `contentSha256` as identity on offline validators, and stored a Compose-chosen container NAME under `containerId` on the live secret-mode row; 1.1.8 removes both.
