...
- Fixture README platform-docker-compose-host declared-env table now names `REVERSE_PROXY`, `LOG_DRIVER`, `RELOAD_WINDOW_SECONDS`, `WEB_APP_NAME`, `WEB_LISTEN_PORT` and `WEB_HEALTH_PATH`, matching every variable the probes and the fixture web service read.
