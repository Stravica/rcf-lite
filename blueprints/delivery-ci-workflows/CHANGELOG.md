...
Fix pass (2026-09-11, criterion e closure): node-gate-entrypoint fails on an empty template set, checks per-JOB that exactly one 'node scripts/rcf-*.js' invocation is present, and OBSERVES an aggregate report's runner.entryPoint by spawning a stub of the named entry and reading the pipeline.json back. workflow-template-shape honours the declared RCF_FIXTURE_CIW_ACTIONLINT_PATH override, records an honest SKIP naming that variable when actionlint is unavailable, and files AC-6101-2 (branch-protection refusal) as an honest AMBER with a named unobservable reason - no longer misbound to actionlint. real-account-github-actions-run-record filters runs to those tied to the blueprint's OWN shipped workflow templates, requires conclusion === 'success' in the predicate, treats gh auth failure with CI_HAS_GITHUB_ACTIONS=true as FAIL (not skip), and skips honestly naming RCF_FIXTURE_CIW_REPO when unset. No default repo (no committed tenant material).
