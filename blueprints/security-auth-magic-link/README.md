...
> Latest: **v1.2.0 (doc-only)** (visual round, 2026-09-06). `capabilities: [principalDirectory]`. See [CHANGELOG.md](CHANGELOG.md).

## Apply

```
rcf define blueprint add <path-to>/blueprints/security-auth-magic-link
```

Phase 1 resolves local path sources only; registry and git-ref resolution is a mechanism follow-up. Apply is idempotent; `rcf define blueprint list` shows the applied entry; `rcf define blueprint remove security-auth-magic-link` cleanly removes an unreferenced application.
