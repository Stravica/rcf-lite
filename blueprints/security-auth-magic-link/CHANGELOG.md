...
- Closure fix pass (Codex 2026-09-11 DO NOT MERGE): re-anchor token-issue-verify to AC-3102-1 (single-use replay) + AC-3102-2 (expired) + AC-3102-3 (default TTL is fifteen minutes; the manager now exposes `ttlSeconds` at runtime and the probe asserts 900). Token-entropy probe samples 1,000 handles (matching AC-3103-1 literally, previously 200) and asserts base64url shape + AC-3103-2 (no JWT-shaped delimiter). Real-account send re-anchored to AC-3110-1 (adapter returns providerMessageId shape), with per-row `evidence` object on the live-Resend result (previously hoisted to the report top level). Gate now distinguishes unset from set-but-not-`true`. Register: round-6 label removed from magic-link-manager comment. `aggregate([])` and null runShim results fail with `no checks ran`. Anatomy test asserts one of the four 7d evidence shapes or an honest skip on every result row.
