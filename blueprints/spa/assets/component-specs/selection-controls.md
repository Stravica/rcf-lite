...
Contracts for select, checkbox, radio group, switch, and combobox (spa-US-1109). All render default, hover, focus, disabled, and error states from tokens in both themes, and meet the 24x24 target including label hit area.
...
Role switch with checked state exposed; Space and Enter toggle (AC-1109-4). Applies immediately, no submit; a pending toggle shows busy and reverts visibly on failure (spa-US-1120 optimistic contract). Label states the thing controlled, not the current value. On and off are distinguishable without colour alone: thumb position plus track token.
