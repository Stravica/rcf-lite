...
Not a log-shipping surface. The application-api-rest blueprint's `logging` topic governs the wire-log shape of the HTTP tier; this blueprint does not extend or conflict with that. Notification outcome records are a durable record surface, not a log.
...
Reach for the observability-essentials blueprint when:
...
Do not reach for the observability-essentials blueprint when:
