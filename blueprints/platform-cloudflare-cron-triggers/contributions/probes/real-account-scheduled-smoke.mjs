  ...
  if (!accountId || !workerName || !token) {
    return {
      results: [{
        anchorAcId: 'AC-32107-1',
        verdict: 'fail',
        detail: `CI_HAS_CLOUDFLARE_ACCOUNT=true but one of CF_ACCOUNT_ID/CF_WORKER_NAME/CF_API_TOKEN is missing: accountIdPresent=${!!accountId} workerNamePresent=${!!workerName} tokenPresent=${!!token}`,
      }],
    };
  }
