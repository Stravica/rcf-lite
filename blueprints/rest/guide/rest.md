...
Any project whose primary surface is an HTTP API meant to be consumed by clients you do not control end to end: a backend for an SPA, a service in a fleet, a public API. Compose it freely with the SPA blueprint; that composition is the intended shape for a product with both tiers.
...
3. Composition conflicts. Adding this blueprint alongside the SPA blueprint surfaces deliberate conflicts on `errorEnvelope` and `authModel`. That is the mechanism working: resolve each with one project-level decision (RFC 7807 end to end; one credential transport mapped onto the four classes).
