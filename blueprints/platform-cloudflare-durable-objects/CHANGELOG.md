...
- `AC-33108-5` reworded to what the cited vendor page actually carries: "key-value backed namespaces can no longer be created; only new_sqlite_classes mints a fresh namespace". The earlier prose specifics (an HTTP-403 status and a cutover-date claim) are dropped from AC and CHANGELOG surfaces because the cited page does not state them.
- Probe `wrangler-seam.mjs` comment reworded to align with the cited page (drops the internal cutover date and HTTP code claims); ADR-3402 context reworded to drop the internal lane reference.
