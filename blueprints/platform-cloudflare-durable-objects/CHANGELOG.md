...
- `AC-33108-1` gains `vendorCitation` to the Cloudflare Durable Objects migrations page (verifiedOn 2026-09-09) so the vendor-fact leader carries a citation alongside `AC-33108-2..4`.
...
- Probe `wrangler-seam.mjs` comment reworded to align with the cited page (drops the internal cutover date and HTTP code claims); ADR-3402 context de-personified ("WSD lane" removed).
- Register sweep: every guide, README, ADR, blueprint.json, US, AC, TAC and probe comment surface on the blueprint has internal operator names, host names, work-item and dispatch ids, and train labels neutralised to concept ("a follow-up", "the throwaway prefix", "the shipped default"). CHANGELOG entries keep internal provenance.
