...
| edge-cloudflare-tunnel-REQ-003 | No public origin ports on the host;  firewall stays closed on 80/443 unless a reverse proxy is applied; manifest refuses ingress rules whose service URL binds to a host public interface. |
...
- **compose-service** (default when `containerHost` is applied). The
  ...
  logging driver aligned with the compose-host log-driver elicit.
...
Graceful degrade: when Access is not applied (absent from
...
The firewall on a `cloudHost` allows 22 unconditionally and 80/443
only when a reverse proxy is applied. A project applying only plus
 (no reverse proxy) declares zero 80/443 rules; the tunnel is the
...
- `compose-service/compose-fragment.yaml`: the cloudflared compose
  service alongside the web and caddy services.
