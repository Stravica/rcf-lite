...
- Record version writer + walker rule (v1.1.14). `writeReport` in `blueprints/deploy-hetzner-server/contributions/probes/probe-utils.mjs` now emits a `version` field on every record, read from this blueprint's own `blueprint.json` at write time (never a literal). The strict walker in `packages/rcf-lite/test/blueprint/deploy-hetzner-server-anatomy.test.js` compares each present record's `version` to `blueprint.json`'s `version` and FAILS on any mismatch; a synthetic in-memory negative case proves a record at an older version is REJECTED without touching `.rcf/reports/`. Under the operator-side override (`RCF_LITE_RECORDS_DIR` set) an expected version may be attested via `RCF_LITE_EXPECTED_VERSION`; CI never sets either variable and the CI walk stays strict against `blueprint.json`.
- Walker error path narrowed to ENOENT (v1.1.14). The walker's `readdir` catch now returns `no local records` ONLY when the directory is absent (`ENOENT`) or has no record files. Every other `readdir` or `readFile` error (permission, I/O, not-a-directory, unparsable JSON) fails the test with the underlying error rather than being silently swallowed as it was before. Paired synthetic negative cases prove a non-directory target and a malformed JSON record each surface as failures.
- Event correlation is identifier-field-equality (v1.1.14). The walker's `serverId` `eventTrail` correlation no longer accepts a substring match over the stringified event body: an event correlates only when its own `serverId` identifier field is present and equal by value. A synthetic negative case proves an event whose `name` or `detail` carries the digits of the id but whose `serverId` field differs (or is absent) does NOT correlate; a positive control and a missing-field control accompany it.
- CHANGELOG history restored (v1.1.14). The 1.1.11 section carries the true 1.1.11 text; a new 1.1.12 section carries the pass-10 content corrected (no vendor-minting claim, describing what the code proves); the 1.1.13 section is unchanged; and this 1.1.14 top entry describes the four items above. `blueprint.json` bumps to 1.1.14 with the anatomy pin in lockstep; `updatedAt` refreshed. Family records regenerated through the shipped run path with no account variables set so each carries the new `version` and the strict walker validates all six.
...
- The mock-purity test title retained for its chain pointer in `packages/rcf-lite/rcf/test-suites/ts-175.json` is left in place; every other anatomy title is a plain descriptive string. A follow-up work item covers the ts-175 pointer rework.
