...
- Closure fix pass (Codex 2026-09-11 DO NOT MERGE): re-anchor every SOPS engine probe to REQ-002 (Secrets are read through one vendor-agnostic manager client interface); no AC covers SOPS byte-equality, rotation MAC divergence or mismatched-key refusal directly, so anchoring the REQ per closure rule 1. Encrypt/decrypt round-trip now uses `sops --input-type binary --output-type binary` with `--output <file>` and compares raw bytes (SHA-256 of both streams; original vs decrypted), replacing the JSON-parse compare that hid formatting drift. Sops child processes now receive a minimal env (`SOPS_AGE_KEY_FILE`, `PATH`, `HOME`) instead of a `...process.env` spread; `PATH` and `HOME` added to `DECLARED_ENV` and to the fixture README env-var table. Key-rotation probe widens the MAC excerpt so before/after divergence is visible in the run record. Register: absolute machine path removed from probe-utils. `aggregate([])` and null runShim results fail with `no checks ran`. Anatomy test tightened the version pin from any 1.1.x to exactly 1.1.3 and asserts one of the four 7d evidence shapes or an honest skip on every result row.
