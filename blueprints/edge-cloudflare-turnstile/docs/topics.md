# edge-cloudflare-turnstile blueprint coordination vocabulary

This file is the edge-cloudflare-turnstile half of the cross-blueprint contract. The Phase 1 conflict detector matches scope:global ADR topics by EXACT string equality, and AC ids are unnamespaced by the 0.4.4 grammar. Any blueprint intended to compose with this one must reuse these exact strings and respect these bands.

## Global ADR topics this blueprint contributes (exact strings)

| Topic string | edge-cloudflare-turnstile contribution | Origin | Composition note |
|---|---|---|---|
| `humanVerificationGate` | ADR-3601-edge-cloudflare-turnstile-verifier-contract | Minted by this blueprint. Names the wire-shape contract for the project-wide human verification gate; the shipped answer is Cloudflare Turnstile with server-side siteverify per `https://developers.cloudflare.com/turnstile/`. | The one project-wide decision on the human verification gate at the shipped-shape boundary. A future non-Cloudflare sibling (hCaptcha, reCAPTCHA) would contribute the same topic string with a different answer, forcing a DELIBERATE conflict the operator resolves with a project-level ADR. |

The edge-cloudflare-turnstile blueprint claims one global topic (minted here, shipped as the Cloudflare Turnstile siteverify verifier). ADR-3602 widget mode and ADR-3603 refuse shape do not contribute global topics; a composing blueprint that holds an opinion on either authors its own project-level ADR.
...
- Reuse `humanVerificationGate` exactly as spelled here when your blueprint holds an opinion on the human verification gate contract; contribute your own scope:global ADR on that string and let composition surface the pairing.
- Consumer blueprints reading the `humanCheck` capability declared on `blueprint.json` per section 6a of `blueprint-authoring.md` obtain the Turnstile-guarded composition at apply time via the capability-declaration mechanism.

## Deliberate-conflict statement

This blueprint's ADR-3601 on `humanVerificationGate` is the first shipped answer to a topic that will conflict, by design, with future non-Cloudflare siblings. Applying `edge-cloudflare-turnstile` alongside such a sibling would raise a `globalAdrTopic` conflict the operator resolves via one of the four documented resolutions (adopt one, keep the existing one, author a project-level supersede ADR, or `--resolve <topic>=project:<ADR-id>` on the add).
