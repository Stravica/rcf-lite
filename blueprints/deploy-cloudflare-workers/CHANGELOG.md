...
Adds deliveredBy on nine REQs (001, 003, 005, 006, 007, 010, 011, 013, 014). Adds ownerRef on AC-12102-2/3 and AC-12109-1 (deployAdapter) and AC-12105-1 (productionUrl). AC-12101-4 rests on the boolean `run_worker_first = true` form with vendor citation https://developers.cloudflare.com/workers/static-assets/binding/. TAC-1302 carries responsibilities for the `[assets].directory` branches, the boolean and array-of-patterns `run_worker_first` mappings, and the `pages_build_output_dir` mutual-exclusion refusal. REQ-002 and TAC-1301 carry the stable `deployStatus` verb that composes production URL, preview alias URL, and currently serving version id. ADR-1303 spelling aligns with TAC-1303. Adds disposition to every AC.
...
shipped SPA-on-Workers deploy shape, per the Cloudflare Pages
...
- ADR `ADR-1306-deploy-cloudflare-workers-spa-shape`: Workers-with-
  static-assets as the shipped SPA-on-Workers deploy shape.
...
Widens the run-worker-first elicit from kind boolean to kind string so both wrangler-side shapes accepted by the Cloudflare Workers static-assets binding (the boolean form and the array-of-patterns form) round-trip through the operator surface without silent narrowing. TAC-1302 responsibility 8 is rewritten to name both shapes with the emitted wrangler literal for each. Adds AC-12113-4 binding the array-of-patterns form on US-12113 with vendorCitation, and adds vendorCitation to AC-12113-2 (boolean form). README and guide teach both shapes. Adds deliveredBy on REQ-002, REQ-004, REQ-008, REQ-009, REQ-012. AC-12102-2, AC-12102-3, AC-12108-1, AC-12108-3, AC-12111-1, and AC-12112-3 carry vendorCitation.
